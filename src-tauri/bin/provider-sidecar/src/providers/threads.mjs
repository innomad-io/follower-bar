...
const PUBLIC_THREADS_USER_AGENT =
...
export async function fetchProfile(payload) {
  ...
  if (!username) {
    return {
      ok: false,
      code: "INVALID_INPUT",
      message: "Invalid Threads handle",
    };
  }

  const browser = await chromium.launch({ headless: true });
  try {
    const page = await browser.newPage({ userAgent: PUBLIC_THREADS_USER_AGENT });
    ...
    if (!profile) {
      return {
        ...
        message: "Threads public page parse failed",
      };
    }
...
function extractProfile({ title, finalUrl, bodyText, metaDescription }) {
  ...
  const titleMatch = String(title ?? "").match(/^(.*?)\s+\(@([A-Za-z0-9._]+)\)\s+•\s+Threads/i);
